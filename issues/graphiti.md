---
title: "Add Graphiti memory provider"
labels: provider, help wanted, good first issue
---

## Goal

Add a `graphiti` memory provider so Cliper can store and query repository memory graphs in **Graphiti**.

**Why Graphiti:** Temporal knowledge graphs - Cliper's Timeline/Commit/Release memories map naturally onto Graphiti's time-aware episodes.
...
- [ ] `src/providers/graphiti/provider.ts` implementing the `MemoryProvider` interface
- [ ] `src/providers/graphiti/client.ts` - connection/API layer (graphiti REST API)
- [ ] `isConfigured()` reads credentials from `~/.cliper/config.json`
- [ ] `cliper auth graphiti` prompt flow (see `src/config/wizard.ts` for the pattern)
...
1. `cliper init` on a small repo stores all memories in Graphiti
